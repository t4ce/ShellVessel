...
use shvessel::job::{JobId, JobTimeout, PlatformJob, PlatformRuntime};
...
fn main() {
    ...
    demo_commands_runtime(&mut vessel, echo_job, foobar_job, mycommand_job);

    // Jobs can be forgotton alltogether, or individually 
    vessel.clean_all();
}

// Demo to help integrate

fn demo_commands_runtime(
    vessel: &mut Vessel<MAX_CMDS, MAX_JOBS>,
    echo_job: Result<JobId, ReturnCodes>,
    foobar_job: Result<JobId, ReturnCodes>,
    mycommand_job: Result<JobId, ReturnCodes>,
) {
    let waker = std::task::Waker::from(std::sync::Arc::new(StdWake));
    let mut context = Context::from_waker(&waker);
    let mut runtime = StdRuntime;

    // every command execution returns a job, and every job can be checked with poll_job
    // sync or async
...
// This part is here aswell only to give the most minimal actual implementation
// That is entirely to your Platform and just a reference to build and test the Shell Vessel Library.

// So here is no deeper meaning, you would need to accustom that to your Platform.
struct StdWake;
